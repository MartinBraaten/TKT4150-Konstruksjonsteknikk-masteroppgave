...
data = pd.read_excel(r'D:\.master\Master\malinger.xls', sheet_name='Nonknots') # load data set
data1 = pd.read_excel(r'D:\.master\Master\malinger.xls', sheet_name='Furu plot') # load data set

# rette opp her!
X2 = data.iloc[1: 12,0].values  # values converts it into a numpy array
Y2 = data.iloc[1: 12,1].values  # -1 means that calculate the dimension of rows, but have 1 column
X2 = np.array(X2)
Y2 = np.array(Y2)
...
X1 = np.array(1.31718588859316 *X1) #1.31718588859316 *
Y1 = np.array(Y1)
X1 = X1.astype(float)
Y1 = Y1.astype(float)
